...
namespace UserManagement
{
    ...
    {
        private UserDetailSalary model = new UserDetailSalary();
        private int Id;
        ...
        public FrmUserSalaryDetail(int id, DateTime startDate,DateTime endDate)
        {
            InitializeComponent();
            Id = id;
            model.Id = Id;
        ...
        private void FrmUserSalaryDetail_Load(object sender, EventArgs e)
        {
            ...
            model.DateWorks = Extendsion.DateWorkByUser(Id, StartDate, EndDate);
            ...
            model.Bonus = Extendsion.ListBonusByUser(Id, StartDate, EndDate);
            //lấy ra danh sách PHẠT
            model.Penaltys = Extendsion.ListPenaltyByUser(Id, StartDate, EndDate);
            //lấy ra danh sách ỨNG LƯƠNG
            model.SalaryAdvances = Extendsion.ListAdvanceByUser(Id, StartDate, EndDate);
            ...
            if (tb.Rows.Count != 0)
            {
                GridUserBonus.DataSource = model.Bonus.Select(c=> new { NameBonus=c.NameBonus, ValueBonus=c.ValueBonus,DateBonus=c.DateBonus }).ToList();
                ...
                foreach (DataRow data in tb.Rows)
                {
                    ...
                    lblIdUser.Text = "Mã nhân viên : " + model.Id;
        ...
        {
            string LocationExcel = Extendsion.GetLocationForExcel();
            if(string.IsNullOrEmpty(LocationExcel))
            {
                return;
            }
            var result = Extendsion.ReportSalaryProfilesUser(model, LocationExcel,StartDate,EndDate, RealSalary, TotalAdvancedP,TotalBonusP,TotalPenaltyP);
            if (result == true)
            {
        ...
        private void DrgidPenalty_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
